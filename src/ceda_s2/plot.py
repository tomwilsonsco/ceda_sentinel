...
class ImagePlotter:
    def __init__(self, gdf, link_col="image_links", band_indices=(1, 2, 3)):
    ...
    def _extract_date_from_link(image_link):
        ...
        try:
            ...
            return file_name, formatted_date
        except Exception as e:
            print(f"Error extracting date from {image_link}: {e}")
            return file_name, None

    def read_from_row(self, gdf_row):
        s2_link = gdf_row[self.link_col]
        file_name, img_date = self._extract_date_from_link(s2_link)
        ...
        with rio.open(s2_link) as src:
            ...
            prof = src.profile
        return window_data, prof, window, file_name, img_date
    ...
    def plot_sample_image(self):
        ...
        window_data, prof, window, file_name, img_date = self.read_from_row(current_row)
        ...
        if len(window_data.shape) == 3 and window_data.shape[0] >= 3:
            ...
            self.ax.set_title(f"{img_date} {file_name}")
            plt.draw()
